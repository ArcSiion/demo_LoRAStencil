...
On RTX A6000, build for `sm_86` and optionally enable correctness checks:
```
cmake -S . -B build-sm86-check -DCMAKE_CUDA_ARCHITECTURES=86 -DLORASTENCIL_ENABLE_CHECKS=ON
cmake --build build-sm86-check -j24
```

For a local smoke test on A6000:
```
